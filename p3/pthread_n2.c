...
int main(int argc, char* argv[])
{
    ...
    pthread_t compute_pos[num_threads];
    pthread_t compute_force[num_threads];



	start = clock();

    for (int id = 0; id < num_threads; id++)	indexs[id] = id;
    generate();	

    for (int step = 0; step < total_steps; step++)
    {
    	for (int id = 0; id < num_threads; id++)	pthread_create(&compute_force[id], NULL, cal_force, (void *)&indexs[id]);
    	for (int id = 0; id < num_threads; id++)	pthread_join(compute_force[id], NULL);

    	for (int id = 0; id < num_threads; id++)	pthread_create(&compute_pos[id], NULL, gogogo, (void *)&indexs[id]);
    	for (int id = 0; id < num_threads; id++)	pthread_join(compute_pos[id], NULL);
